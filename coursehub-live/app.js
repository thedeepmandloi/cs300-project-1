const state = {
  currentView: "browse", // browse, bookmarks, about
  searchResults: [],
  bookmarks: JSON.parse(localStorage.getItem("coursehub-live-bookmarks")) || [],
  isLoading: false,
  error: null,
  searchQuery: "",
};
...
function createSearchBar() {
  return `
    ...
    <input type="text" name="query" placeholder="Search books..." value="${state.searchQuery}" />
...
function renderBrowserView() {
  ...
  if (state.isLoading) {
    ...
    content += `<p class="error">Error: ${state.error}</p>`;
...
function render() {
  ...
  const views = {
    browse: renderBrowserView,
    bookmarks: renderBookmarksView,
    about: renderAboutView,
  };

  const viewFn = views[state.currentView] || renderBrowserView;
...
// INITIALIZE
if (location.hash) {
  state.currentView = location.hash.slice(1);
}
render();
